#!/usr/bin/env bash
# Faz 6C-Beta D1 — görsel kabul Playwright (pilot localhost, flag değiştirmez)
set -Eeuo pipefail
...
SHOT_DIR="/var/log/acarindex-d1-visual-acceptance"
...
HESABIM_ENV="/tmp/faz6c-d1-hesabim.env"
...
docker run --rm \
  ...
  -e ACAR_D1_VISUAL_SHOTS="$SHOT_DIR" \
  --network host \
  "$PLAYWRIGHT_IMAGE" \
  bash -c 'npm install @playwright/test@1.49.1 --no-save && npx playwright test --config=playwright.beta-d1.config.ts'

rm -f "$HESABIM_ENV"

echo "SHOT_DIR=$SHOT_DIR"
echo "D1_VISUAL_PLAYWRIGHT_OK"
