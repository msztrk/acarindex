...
echo "=== RESTORE PRE-B1 SCHEMA (no catalog data — disk-safe) ==="
...
$ACAR_COMPOSE exec -T postgres psql -U acarindex_pilot -d "$REHEARSAL_DB" -c \
  "INSERT INTO content_applications (id, kind, user_id, title, status)
   VALUES ('00000000-0000-4000-8000-0000000000b2', 'new_journal', '00000000-0000-4000-8000-0000000000b1', 'B1 Rehearsal', 'draft')
   ON CONFLICT (id) DO NOTHING;"

CONTENT_ID='00000000-0000-4000-8000-0000000000b2'
...
POST_CONTENT=$($ACAR_COMPOSE exec -T postgres psql -U acarindex_pilot -d "$REHEARSAL_DB" -qAt -c \
  "SELECT count(*) FROM content_applications WHERE id = '$CONTENT_ID';" | head -1 | tr -d '\r\n')
...
[[ "$POST_CONTENT" == "1" ]] || { echo "FAIL: rehearsal content row missing after rollback"; exit 1; }
